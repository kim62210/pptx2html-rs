...
- Improve line breaking beyond the newly covered adjacent-run, paragraph-default-size, inherited-style-size, `spAutoFit` long-token, partial `normAutofit` inheritance, NBSP non-breaking, soft-hyphen discretionary-break, fullwidth East Asian-form, CJK non-starter punctuation, slash-/hyphen-separated token, and CJK opening-punctuation cases, especially in harder mixed-font, mixed-script, and autofit scenarios.
...
**Suggested slices:**
1. Add another deterministic line-breaking regression beyond the adjacent-run, paragraph-default-size, inherited-style-size, `spAutoFit` long-token, partial `normAutofit` inheritance, NBSP non-breaking, soft-hyphen discretionary-break, fullwidth East Asian-form, CJK non-starter punctuation, slash-/hyphen-separated token, and CJK opening-punctuation cases.
