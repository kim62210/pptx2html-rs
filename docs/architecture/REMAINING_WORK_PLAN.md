...
- **Text**: rendered and significantly improved, with adjacent-run unbreakable token detection and paragraph-level default font sizes now feeding emergency-wrap decisions, but still `approximate` until measurement-driven fidelity and PowerPoint-reference verification are expanded.
...
- Improve line breaking beyond the newly covered adjacent-run and paragraph-default-size cases, especially in harder mixed-font, mixed-script, and autofit scenarios.
...
**Suggested slices:**
1. Add another deterministic line-breaking regression beyond the adjacent-run and paragraph-default-size cases.
